...
export default function Labels() {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success("Logo transmitted successfully.");
    } catch (err: any) {
      toast.error("Logo transmission failed: " + (err.message || "Unknown error"));
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingLabel) {
        ...
        toast.success("Label asset updated.");
      } else {
        ...
        toast.success("New corporate entity established.");
      }
      setShowModal(false);
    ...
    <div className="space-y-12 pb-20 text-left">
      ...
      {error && (
        <motion.div 
          ...
          className="bg-amber-50 border border-amber-200 p-6 rounded-[2rem] flex items-center gap-4 text-amber-700 mx-2"
        >
